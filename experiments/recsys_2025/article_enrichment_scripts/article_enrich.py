...
This script enriches news dataset entries based on a selected configuration.

HOW TO USE:
- Choose the appropriate dataset configuration between one of the following three:
...
IMPORTANT:
- This script works the same for the three datasets, but make sure:
    - The config file you import defines the correct path to the input CSV
    - The desired output path is also set in the config


- In this script's `enhance_data` function, only the first 10 rows of the dataset are processed:
    df = df[:10]
    - This helps users get familiar with the flow before processing the full dataset.
  To process everything, simply remove or comment out that line.

    - Estimated time for full dataset runs can be several hours depending on dataset size.
...
# NOTE:
# Only change the imported config based on the dataset you're working with.
# The rest of the code will work the same for all three datasets (ebnerd, nemig, mind).
# Before running this script, make sure the selected config file correctly specifies:
# - The path to the required CSV input
# - The desired output path for saving results
...
def enhance_data(input_file_path, output_file_path):
    """Enhance dataset based on configuration and save the enriched data.
      Notes:
        - This function currently processes only the first 10 rows of the input data,
          as a quick test/demo to help users understand how the enrichment process works.
        - Processing the entire dataset can take several hours, depending on size and complexity.
        - To process the full dataset, remove or comment out the line: `df = df[:10]`.
...
if __name__ == "__main__":
    ...
    print(f"Dataset successfully enhanced! Check {config.output_file_path}")