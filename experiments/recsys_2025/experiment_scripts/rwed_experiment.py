...
user_num = len(set([u for u,i,r in feedback_train]))
item_num = len(set([i for u,i,r in feedback_train]))

# Define metrics
targetSize = 20
metrics = [Recall(k=targetSize)]

model = RWE_D(article_pool = impression_iid_list)

experiment_output_path = f'./experiment_{dataset_name}_rwed_results'
