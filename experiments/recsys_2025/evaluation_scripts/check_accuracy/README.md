...
| Script | Purpose                                                                                                                                                                                                |
|--------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `run_accuracy_metrics.py` | CLI runner for AUC + P@K + R@K + NDCG@K. Supports both `item_scores.pkl` (Mode A) and ranked `.txt` files (Mode B). Evaluates one config, a selection, or all configs in one go. |
| `compute_auc.py` | Standalone — computes AUC only from `item_scores.pkl`.                                                                                                          |
...
## Running `run_accuracy_metrics.py`

The `run_accuracy_metrics.py` method only works for the D-RDW algorithm. If another model needs to be evaluated, the 
coded `save_path` should be changed.
...
### Mode A — `item_scores.pkl` (recommended)
...
To generate the required files, please refer to the [experiments README](../../README.md).
...
### Mode B — ranked `.txt` recommendation file
...
## Output for `run_accuracy_metrics.py`
